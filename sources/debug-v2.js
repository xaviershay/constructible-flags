...
function renderFill(fill) {
    ...
    switch (fill.type) {
        ...
        case "crescent": {
            const id =
                `cm_${fill.outerCx}_${fill.outerCy}_${fill.outerR}`.replace(
                    /[^a-zA-Z0-9_]/g,
                    "_",
                );
            return html`
...
const LeafLayer = memo(function LeafLayer({
    ...
    return html`
        <g>
            ${showFill &&
            leaf.fill &&
            html` <g class="fill-layer">${renderFill(leaf.fill)}</g> `}
            <g>
                ${isVisible &&
                leaf.geom &&
                html` <g class="geom-layer">${renderGeom(leaf.geom)}</g> `}
                ${isVisible &&
                html`
                    <g class="dots-layer">
                        ${pts.map(
...
function SvgViewer({
    ...
    const onPointerDown = useCallback(
        (e) => {
            ...
            if (
                e.button === 1 ||
                (e.button === 0 && !e.target.classList.contains("dot"))
            ) {
                e.preventDefault();
                const svg = svgRef.current;
                if (!svg) return;
    ...
    return html`
        <div class="range-slider">
            <div class="range-slider-track" ref=${trackRef}>
                ...
                />
                <div
                    class="range-slider-thumb range-slider-thumb-lo"
                    style=${{ left: loPos + "%" }}
                ...
                <div
                    class="range-slider-thumb range-slider-thumb-mid"
                    style=${{ left: midPos + "%" }}
                ...
                <div
                    class="range-slider-thumb range-slider-thumb-hi"
                    style=${{ left: hiPos + "%" }}
...
const StepNode = memo(function StepNode({
    ...
    const groupLeaves = useMemo(
        () => (node.type === "group" ? allLeafIndices(node) : []),
        [node],
    );
    ...
    const anyActive = groupLeaves.some(
        (idx) => idx >= loLeafIndex && idx <= hiLeafIndex,
    );
...
function CoordTooltip({ point }) {
    ...
    return html` <div class="coord-tooltip" ref=${ref} /> `;
}
...
function App() {
    ...
    const midpointLeafIndex =
        midVirtual < virtualLayers.length ? virtualLayers[midVirtual][0] : null;
        ...
        <div class="app">
            ...
            <div class="app-body">
                ...
                <div class="tree-panel" ref=${treeRef}>
                    ...
                    <${StepTree}
                        tree=${DATA.tree}
                        loLeafIndex=${lo < virtualLayers.length
                            ? virtualLayers[lo][0]
                            : Infinity}
                        hiLeafIndex=${virtualLayers.length > 0
                            ? Math.max(
                                  ...virtualLayers[
                                      Math.min(hi, virtualLayers.length - 1)
                                  ],
                              )
                            : 0}
