...
import Test.Tasty.QuickCheck (testProperty, forAll, (==>))
...
fieldNumberTests = testGroup "FieldNumber"
  [ constructionTests
  , trigWrapperTests
...
trigWrapperTests = testGroup "Trig wrappers"
...
arithmeticFieldTests = testGroup "Arithmetic field promotion"
  [ testGroup "Addition"
...
sqrtTests = testGroup "sqrt"
  [ testGroup "Perfect-square integers -> FInteger"
  ...
  , testGroup "Non-perfect-square integers -> FIrrational"
  ...
  , testGroup "Perfect-square rationals -> FRational"
  ...
  , testGroup "Non-perfect-square rationals -> FIrrational"
  ...
  , testGroup "Higher fields: field unchanged"
  ...
  , testGroup "sqrt value correctness"
...
predicateTests = testGroup "Predicates"
  [ testGroup "isZero"
...
eqOrdTests :: TestTree
eqOrdTests = testGroup "Eq and Ord"
...
numericCorrectnessTests = testGroup "Numeric correctness"
  ...
  , testCase "Quadratic x^2 - 5x + 6 = 0: roots are FInteger 2 and 3" $ do
      let b = fnInteger (-5); c = fnInteger 6
          disc = sqrt (b*b - 4*c)
          x1 = (-b - disc) / 2
          x2 = (-b + disc) / 2
...
toRationalTests = testGroup "toRational'"
...
displayTests = testGroup "Display"
  ...
  , testCase "showFN includes field tag" $
      assertBool "contains FInteger" ("FInteger" `isInfixOf` showFN (fnInteger 3))

  , testCase "showFN FIrrational includes FIrrational" $
      assertBool "contains FIrrational" ("FIrrational" `isInfixOf` showFN (sqrt (fnInteger 2)))
  ]
...
properties = testGroup "Properties"
  ...
  , testProperty "division of rationals produces rational or integer" $
      forAll ((,) <$> genFieldNumber <*> genFieldNumber) $ \(a, b) ->
        isZero b || fieldOf a > FRational || fieldOf b > FRational ||
