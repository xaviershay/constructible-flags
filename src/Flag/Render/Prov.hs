...
import Flag.Source (Agent (..), Entity (..), Source (..), SourcedElement (..))

-- | Generate a W3C PROV-JSON document for a flag's provenance.
generateProvJson :: String -> String -> [SourcedElement] -> BSL.ByteString
generateProvJson isoCode flagName' sources =
  ...
  where
    ...
    usedRels =
      ...
      | SourcedAttr name _ <- sources,
        name `notElem` intermediateNames
      ]
        ++
        ...
        [ object
            [ "prov:activity" .= translateActivityId e,
    ...
    wdfRels =
      ...
      [ object
          [ "prov:generatedEntity" .= cf (screenshotIdStr e i),
    ...
    attrWasInfluencedBy (SourcedAttr name (SourceImpliedReference e)) =
      [ object
          [ "prov:influencee" .= cf (attrId name),
