...
drawingToElement (CT.DrawTriangle col pt1 pt2 pt3) =
    polygon_ [ Points_        <<- pointsAttr (map toDP [pt1, pt2, pt3])
             , Fill_          <<- colHex col
             , Fill_opacity_  <<- "1"
             , Stroke_        <<- colHex col
...
drawingToElement (CT.DrawPath col pts@(_:_)) =
    polygon_ [ Points_        <<- pointsAttr (map toDP pts)
             , Fill_          <<- colHex col
             , Fill_opacity_  <<- "1"
             , Stroke_        <<- "none"
             , Stroke_width_  <<- "0"
