...
import Flag.Construction.Types (Number, Point)
  ...
  let (x1, y1) = p1
   ...
   in if isZero ((x2 - x1) ^ (2 :: Int) + (y2 - y1) ^ (2 :: Int))
        then error "evalIntersectLL': first line is degenerate (both points are equal)"
          ...
          if isZero ((x4 - x3) ^ (2 :: Int) + (y4 - y3) ^ (2 :: Int))
          ...
          if isZero r2sq
            then error "evalIntersectCC': second circle is degenerate (zero radius: center equals edge point)"
              ...
              if d + min r1 r2 < max r1 r2
                then error "evalIntersectCC': one circle is entirely within the other (no intersection)"
                else
                  ( (mx + hd * dy, my - hd * dx),
                    (mx - hd * dy, my + hd * dx)
                  )
