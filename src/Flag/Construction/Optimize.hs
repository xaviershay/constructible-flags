...
optimize = buildDrawing . processGroups . flatten
  where
    ...
    -- DrawMasked is kept as a unit but its sub-drawings are recursively optimized.
    flatten :: Drawing -> [Drawing]
    flatten EmptyDrawing = []
    flatten (Overlay a b) = flatten a ++ flatten b
    flatten (DrawMasked mode c m) = [DrawMasked mode (optimize c) (optimize m)]
    flatten d = [d]
